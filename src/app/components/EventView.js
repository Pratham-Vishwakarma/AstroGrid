...
const EventView = ({ selectedDate, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-gray-200 border-s-2 border-gray-600">
          ...
          <div className="space-y-4 h-[71vh] overflow-y-auto overflow-x-hidden scrollbar-none px-3 py-2">
            {dayEvents.map((event) => (
              <div 
                key={event.id}
                className={`
                  bg-gray-900 border-l-4 ${event.color}
